...
* **tsify** supports overriding the `files`, `excludes` and `includes` options. In particular, if `"files": []` is specified, only the Browserify entry points (and their dependencies) are passed to TypeScript for compilation.
