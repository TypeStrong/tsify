...
module.exports = function (ts) {
	function Host(currentDirectory, languageVersion, outDir) {
	...
	Host.prototype._normalizeOutput = function(filename) {
		return path.relative(this.outDir, path.resolve(filename)).replace(/\\/g, '/');
	}
	...
	Host.prototype.writeFile = function (filename, data) {
		this.output[this._normalizeOutput(filename)] = data;
	};
