...
module.exports = function (ts) {
	...
	function isTypescriptOrJavascript(file) {
		return isTypescript(file) || isJavascript(file);
	}
	...
	function parseOptions(opts) {
		...
		// Files are written to tsify's in-memory cache instead of the filesystem so these option are
		// not needed for writing files to disk.  However they are needed in order to enable *.js files 
		// to be compiled otherwise the typescript lib would ignore them since they would overwrite their input. 
		delete parsed.options.out;
		parsed.options.rootDir = currentDirectory;
		parsed.options.outDir = path.join(currentDirectory, '../out');
	...
	function Tsifier(opts) {
		...
		self.host = new Host(currentDirectory, this.opts.target, this.opts.outDir);
	...
	Tsifier.prototype.generateCache = function (files) {
		var self = this;
		var tsFiles = files.filter(self.opts.allowJs ? isTypescriptOrJavascript : isTypescript)
	...
	Tsifier.prototype.transform = function (file) {
		...
		if (!isTypescript(file) && !(self.opts.allowJs && isJavascript(file)))
			return through();
